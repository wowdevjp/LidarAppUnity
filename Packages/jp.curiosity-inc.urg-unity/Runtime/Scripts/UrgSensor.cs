...
namespace Urg
{
    public class UrgSensor : MonoBehaviour
    {
        ...
        readonly string COMMAND_REBOOT_SCANNING = "RB";
        ...
        private void Close()
        {
            ...
            {
                thread.Join();
                Debug.LogWarning($"Close:thread.Join();");
            }

            if (transport.IsConnected())
            {
                transport.Close();
                Debug.LogWarning($"Close:transport.Close();");
            }
        }
        ...
        public void RebootScanning()
        {
            Write(COMMAND_REBOOT_SCANNING + "\n");
        }
